...
export function PrivacyModal({ isOpen, onClose, isBreak, language }: PrivacyModalProps) {
  if (!isOpen) return null;

  const t = translations[language];
  const document = t.legal.privacy;
    ...
    >
      ...
      >
        <header className={`flex shrink-0 items-start justify-between gap-4 border-b p-5 sm:p-6 ${isBreak ? "border-neutral-200" : "border-neutral-700"}`}>
          <div>
            <h2 id="privacy-modal-title" className="flex items-center gap-2 text-xl font-black sm:text-2xl">
              <ShieldCheck className={isBreak ? "text-emerald-500" : "text-red-400"} size={23} />
              {document.title}
            </h2>
